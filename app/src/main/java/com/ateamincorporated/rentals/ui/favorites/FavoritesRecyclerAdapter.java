...
public class FavoritesRecyclerAdapter extends RecyclerView.Adapter<FavoritesRecyclerAdapter.ViewHolder> implements Filterable {
     ...
     */
    public FavoritesRecyclerAdapter(HomeViewModel homeViewModel, FavoritesFragment fragment, RecyclerView recyclerView, View emptyView, String uid) {
...
//        homeViewModel.setEmptyView(EMPTY_VIEW_WAIT);
    ...
    public void completeInit() {
        reference.child(DatabaseContract.GLOBAL_ASSET_PATH).addChildEventListener(new ChildEventListener() {
            ...
            public void onChildRemoved(@NonNull DataSnapshot snapshot) {
                mDataIndex.remove(snapshot.getKey());
                mDataSet.remove(snapshot.getValue(Asset.class));
                notifyAdapterDataSetChanged();
            }
    ...
    @NotNull
    @Override
    public FavoritesRecyclerAdapter.ViewHolder onCreateViewHolder(ViewGroup viewGroup, int viewType) {
        // Create a new view.
        View v = LayoutInflater.from(viewGroup.getContext())
                .inflate(R.layout.row_item_home, viewGroup, false);

        if (mUid == null) {
            v.getRootView().findViewById(R.id.favorite).setVisibility(View.INVISIBLE);
        }
    ...
    @Override
    public void onBindViewHolder(@NotNull FavoritesRecyclerAdapter.ViewHolder holder, int position) {
        ...
        if (asset != null) {
            ...
            if (mUid != null) {
                ...
                holder.getFavouriteButton().setOnClickListener(v -> removeFavorite(asset));
            }
        }
    }

    private void removeFavorite(@NotNull Asset asset) {
    ...
    @Override
    public void onBindViewHolder(@NonNull FavoritesRecyclerAdapter.ViewHolder holder, int position, @NonNull List<Object> payloads) {
    ...
    public Filter getFilter(int filterType, String location, String category) {
        ...
        return new Filter() {
            ...
            protected FilterResults performFiltering(CharSequence query) {
                FavoritesRecyclerAdapter.this.query = query;
                String queryString;
                if (query == null) {
                    queryString = "";
                } else {
                ...
                if (queryString.isEmpty() && FavoritesRecyclerAdapter.this.location.isEmpty() && FavoritesRecyclerAdapter.this.category.isEmpty()) {
                    mDataSetFiltered.clear();
                    for (String s : mDataSetFavorite) {
                        if (mDataIndex.contains(s)) {
                            mDataSetFiltered.add(mDataSet.get(mDataIndex.indexOf(s)));
                        }
                    }
                } else {
                    ...
                    for (String s : mDataSetFavorite) {
                        if (mDataIndex.contains(s)) {
                            Asset row = mDataSet.get(mDataIndex.indexOf(s));
                            if (filterType == FILTER_TYPE_CATEGORY && row.category.equals(FavoritesRecyclerAdapter.this.category)) {
                                filteredList.add(row);
                            }

                            if (filterType == FILTER_TYPE_LOCATION && row.genericLocation.equals(FavoritesRecyclerAdapter.this.location)) {
                                filteredList.add(row);
                            }

                            if (filterType == FILTER_TYPE_CATEGORY_LOCATION && row.genericLocation.equals(FavoritesRecyclerAdapter.this.location) && row.category.equals(FavoritesRecyclerAdapter.this.category)) {
                                filteredList.add(row);
                            }
                            ...
                            if (filterType == FILTER_TYPE_CATEGORY_SEARCH && row.name.toLowerCase().contains(queryString.toLowerCase()) && row.category.equals(FavoritesRecyclerAdapter.this.category)) {
                                filteredList.add(row);
                            }

                            if (filterType == FILTER_TYPE_SEARCH_LOCATION && row.name.toLowerCase().contains(queryString.toLowerCase()) && row.genericLocation.equals(FavoritesRecyclerAdapter.this.location)) {
                                filteredList.add(row);
                            }

                            if (filterType == FILTER_TYPE_CATEGORY_SEARCH_LOCATION && row.name.toLowerCase().contains(queryString.toLowerCase()) && row.genericLocation.equals(FavoritesRecyclerAdapter.this.location) && row.category.equals(FavoritesRecyclerAdapter.this.category)) {
                                filteredList.add(row);
                            }
                        }
                    }
...
//                    homeViewModel.setSecondaryTitle(String.format(FavoritesFragment.TITLE_SECONDARY_SEARCH, getItemCount(), charSequence));
//                }
            }
        };
    }
